...
struct GLFWwindow {
    ...
    GLFWwindowrefreshfun        refreshCallback = nullptr;
    GLFWwindowfocusfun          focusCallback = nullptr;
    GLFWwindowiconifyfun        iconifyCallback = nullptr;
    GLFWwindowcontentsizefun    contentScaleCallback = nullptr;
...
GLFWwindowrefreshfun glfwSetWindowRefreshCallback(GLFWwindow* w, GLFWwindowrefreshfun cb) {
    if (!w) return nullptr; auto old = w->refreshCallback; w->refreshCallback = cb; return old;
}
GLFWwindowfocusfun glfwSetWindowFocusCallback(GLFWwindow* w, GLFWwindowfocusfun cb) {
    if (!w) return nullptr; auto old = w->focusCallback; w->focusCallback = cb; return old;
}
GLFWwindowiconifyfun glfwSetWindowIconifyCallback(GLFWwindow* w, GLFWwindowiconifyfun cb) {
    if (!w) return nullptr; auto old = w->iconifyCallback; w->iconifyCallback = cb; return old;
}
GLFWwindowcontentsizefun glfwSetWindowContentScaleCallback(GLFWwindow* w, GLFWwindowcontentsizefun cb) {
    if (!w) return nullptr; auto old = w->contentScaleCallback; w->contentScaleCallback = cb; return old;
}
